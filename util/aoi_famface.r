aoi_famface <- function(df, rowname_pairs) {
  ...
  df <- cbind(AOIFamFace = "NO EVAL", df, stringsAsFactors = FALSE)
  ...
  aoi_left <- matrix(c(177, 177, 727, 627), nrow = 2, ncol = 2)
  aoi_right <- matrix(c(1330, 177, 1880, 627), nrow = 2, ncol = 2)
  ...
  while (length(rowname_pairs) > 0) {
    ...
    for (i in current_row_start_pos:current_row_end_pos) {
      df$AOIFamFace[i] <- aoi_checker(df$GazePointX..ADCSpx.[i], df$GazePointY..ADCSpx.[i])
    }
