aoi_preflook <- function(df, rowname_pairs) {
  ...
  df <- cbind(AOIPrefLook = "NO EVAL", df, stringsAsFactors = FALSE)
  ...
  aoi_left <- matrix(c(350, 396, 710, 756), nrow = 2, ncol = 2)
  aoi_right <- matrix(c(1338, 396, 1698, 756), nrow = 2, ncol = 2)
  ...
  while (length(rowname_pairs) > 0) {
    ...
    for (i in current_row_start_pos:current_row_end_pos) {
      df$AOIPrefLook[i] <- aoi_checker(df$GazePointX..ADCSpx.[i], df$GazePointY..ADCSpx.[i])
    }
