get_trial_offset <- function(df, trial_ranges, trial_offset) {
  ...
  starting_times <- df$timestamp[trial_ranges$start]
  ...
  if (trial_offset[1] != "start") {
    ...
    starting_times <- df$timestamp[trial_ranges$start] + as.numeric(trial_offset[1])
  }

  if (trial_offset[2] != "end") {
    # Add the endtime argument to the starting times
    ending_times <- df$timestamp[trial_ranges$start] + as.numeric(trial_offset[2])
  }
