...
# !diagnostics suppress=mc, coi, recs_dir, lut_filename, lut_fam_phase, inter_trial_chunk_patterns, aoi_fam_body_object, aoi_fam_face, aoi_preflook, aoi_screen
source("interface.r")
...
for (i in 1:length(flist)) {
  ...
  # read tsv files
  df <- read.table(file = file.path(recs_dir, current_subject), sep = "\t", header = TRUE)

  # get start and end index pairs for inter_trial chunks
  familiarization_attention_startend <- getStartEndPositions(df, inter_trial_chunk_patterns[1], "MovieStart", "MovieEnd")
  familiarization_startend <- getStartEndPositions(df, inter_trial_chunk_patterns[2], "MovieStart", "MovieEnd")
  preflook_attention_startend <- getStartEndPositions(df, inter_trial_chunk_patterns[3], "MovieStart", "MovieEnd")
  preflook_startend <- getStartEndPositions(df, inter_trial_chunk_patterns[4], "MovieStart", "MovieEnd")



  # Allocate Trials and Fillup StudioEventData Label (todo: this df is not necessary)
  df <- df
  df <- allocateTrials(df, familiarization_attention_startend) # keep this enabled to track valid trials
  df <- allocateTrials(df, familiarization_startend)
  df <- allocateTrials(df, preflook_attention_startend) # keep this enabled to track valid trials
  df <- allocateTrials(df, preflook_startend)
  ...
  inter_vectors <- as.character(unique(df$StudioEventData[familiarization_startend$start]))
  ...
  df <- getAOIs(df, aoi_fam_body_object, familiarization_startend)
  # AOI column for Familiarization Phase for faces (left & right)
  df <- getAOIs(df, aoi_fam_face, familiarization_startend)
  # AOI column for Preferential Looking Phase for objects (left & right)
  df <- getAOIs(df, aoi_preflook, preflook_startend)
  # AOI column for Familiarization Phase & Preferential Looking Phase for screen (TRUE/FALSE)
  df <- getAOIs(df, aoi_screen, c(familiarization_startend, preflook_startend))
  ...
  dfX_base$Overall_LT_ScreenFam <- getLooks(df, aoi_screen, familiarization_startend)$looking_times
  dfX_base$Overall_LT_ScreenPrefLook <- getLooks(df, aoi_screen, preflook_startend)$looking_times
  ...
  dfX_base$InterPhase_LT_Soc_Begin <- getLooks(df, aoi_screen, familiarization_startend, c(2000, 3000))$looking_times
  dfX_base$InterPhase_LT_Soc_End <- getLooks(df, aoi_screen, familiarization_startend, c(10000, 11000))$looking_times
  ...
  dfX_base$InterPhase_LT_Gazing <- getLooks(df, aoi_screen, familiarization_startend, c(4000, 9000))$looking_times

  # AOIs: Looking times over TOTAL duration of the video
  dfX_base$Inter_LT_ActorLeft <- getLooks(df, aoi_fam_body_object, familiarization_startend)$looking_times$left
  dfX_base$Inter_LT_ActorRight <- getLooks(df, aoi_fam_body_object, familiarization_startend)$looking_times$right
  ...
  dfX_base$Inter_LT_FaceLeft <- getLooks(df, aoi_fam_face, familiarization_startend)$looking_times$left
  dfX_base$Inter_LT_FaceRight <- getLooks(df, aoi_fam_face, familiarization_startend)$looking_times$right
  ...
  dfX_base$Inter_LT_Object <- getLooks(df, aoi_fam_body_object, familiarization_startend)$looking_times$center
  ...
  dfX_base$InterPhase_LT_FaceLeft_Soc_Begin <- getLooks(df, aoi_fam_face, familiarization_startend, c(2000, 3000))$looking_times$left
  dfX_base$InterPhase_LT_FaceRight_Soc_Begin <- getLooks(df, aoi_fam_face, familiarization_startend, c(2000, 3000))$looking_times$right
  ...
  dfX_base$InterPhase_LT_FaceLeft_Soc_End <- getLooks(df, aoi_fam_face, familiarization_startend, c(10000, 11000))$looking_times$left
  dfX_base$InterPhase_LT_FaceRight_Soc_End <- getLooks(df, aoi_fam_face, familiarization_startend, c(10000, 11000))$looking_times$right
  ...
  dfX_base$InterPhase_LT_Object_Soc_Begin <- getLooks(df, aoi_fam_body_object, familiarization_startend, c(2000, 3000))$looking_times$center
  dfX_base$InterPhase_LT_Object_Soc_Begin_PROP <- dfX_base$InterPhase_LT_Object_Soc_Begin / dfX_base$InterPhase_LT_Soc_Total
  dfX_base$InterPhase_LT_Object_Soc_End <- getLooks(df, aoi_fam_body_object, familiarization_startend, c(10000, 11000))$looking_times$center
  ...
  dfX_base$InterPhase_LT_FaceLeft_Gazing <- getLooks(df, aoi_fam_face, familiarization_startend, c(4000, 9000))$looking_times$left
  dfX_base$InterPhase_LT_FaceRight_Gazing <- getLooks(df, aoi_fam_face, familiarization_startend, c(4000, 9000))$looking_times$right
  ...
  dfX_base$InterPhase_LT_Object_Gazing <- getLooks(df, aoi_fam_body_object, familiarization_startend, c(4000, 9000))$looking_times$center
  ...
  # -------------------------------------------------------------------------------------------
  dfX_base$PrefLook_Object_Fam <- getObjects(df, familiarization_startend)$familiar
  dfX_base$PrefLook_Object_Fam_Pos <- getPrefLookPositions(as.vector(unique(df$StudioEventData[preflook_startend$start])), getObjects(df, familiarization_startend)$familiar)$fam_pos
  dfX_base$PrefLook_Object_Nov <- getObjects(df, familiarization_startend)$novel
  dfX_base$PrefLook_Object_Nov_Pos <- getPrefLookPositions(as.vector(unique(df$StudioEventData[preflook_startend$start])), getObjects(df, familiarization_startend)$familiar)$nov_pos
  dfX_base$PrefLook_LT_Object_Left <- getLooks(df, aoi_preflook, preflook_startend)$looking_times$left
  dfX_base$PrefLook_LT_Object_Right <- getLooks(df, aoi_preflook, preflook_startend)$looking_times$right
  ...
  # First Looks
  # -------------------------------------------------------------------------------------------
  dfX_base$PrefLook_FL <- getLooks(df, aoi_preflook, preflook_startend)$first_look
